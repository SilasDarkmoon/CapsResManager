	...
	IEnumerable StringifyAsync(int depth, StringBuilder builder, bool pretty = false) {	//Convert the JSONObject into a string
		...
		switch(type) {
			case Type.BAKED:
				...
				break;
			case Type.STRING:
                builder.AppendFormat("\"{0}\"", ToJSONString(str));
				break;
			case Type.NUMBER:
//#if USEFLOAT
...
//#endif
		...
		if(type == Type.OBJECT) {
			...
			for(int i = 0; i < list.Count; i++) {
				...
				switch(val.type) {
					...
					case Type.NUMBER: result.Add(keys[i], val.n.ToString()); break;
					case Type.BOOL: result.Add(keys[i], val.b.ToString()); break;
    ...
    public object ToObject()
    {
        if (type == Type.OBJECT)
        {
            ...
            {
                JSONObject val = list[i];
                result.Add(keys[i], val.ToObject());
            }
            return result;
        }
