...
using static Capstones.UnityEditorEx.CapsResBuilder;
...
namespace Capstones.UnityEditorEx
{
    public static class CapsResBuilder
    {
        ...
        public interface ICapsResBuildWork
        {
            string Phase { get; set; }
            string OutputDir { get; set; }

            IBundleBuildInfo[] BundleBuildInfos { get; set; }
            CapsResManifest[] Manifests { get; set; }
        ...
        public interface IResBuilderEx
        {
            ...
            void GenerateBuildWork(string bundleName, IList<string> assets, IBundleBuildInfo bundlework, ICapsResBuildWork modwork, int bindex);
            void PostBuildWork(string mod, ICapsResBuildWork work, string dest);
            void Cleanup();
            void OnSuccess();
        }
        ...
        public abstract class BaseResBuilderEx : IResBuilderEx
        {
            ...
            void IResBuilderEx.GenerateBuildWork(string bundleName, IList<string> assets, IBundleBuildInfo bundlework, ICapsResBuildWork modwork, int bindex)
            {
            }

            void IResBuilderEx.PostBuildWork(string mod, ICapsResBuildWork work, string dest)
            {
            }
        }
        public abstract class BaseResBuilderEx<T> : BaseResBuilderEx, IResBuilderEx where T : BaseResBuilderEx<T>, new()
        {
            public virtual void GenerateBuildWork(string bundleName, IList<string> assets, IBundleBuildInfo bundlework, ICapsResBuildWork modwork, int bindex)
            {
            }

            public virtual void PostBuildWork(string mod, ICapsResBuildWork work, string dest)
            {
            }
        ...
        public interface ICapsResBuilderImp
        {
            string BundleExtName { get; }
            ICapsResBuildWork CreateWork();
            IBundleBuildInfo CreateBundleBuildInfo();
            TaskProgress Prepare(string outputdir);
            void Cleanup(ICapsResBuildWork work);
            void Build(ICapsResBuildWork work);
            Dictionary<string, IList<string>> GroupBuiltFiles(string outputdir);
        }
        public static ICapsResBuilderImp BuilderImp;

        public static IEnumerator GenerateBuildWorkAsync(Dictionary<string, ICapsResBuildWork> result, IList<string> assets, IEditorWorkProgressShower winprog)
        {
            return GenerateBuildWorkAsync(result, assets, winprog, null);
        }
        public static IEnumerator GenerateBuildWorkAsync(Dictionary<string, ICapsResBuildWork> result, IList<string> assets, IEditorWorkProgressShower winprog, IList<IResBuilderEx> runOnceExBuilder)
        {
            var logger = new EditorWorkProgressLogger() { Shower = winprog };
            ...
            if (assets != null)
            {
                ...
                foreach (var kvpbuild in mod2build)
                {
                    ...
                    ICapsResBuildWork work = BuilderImp.CreateWork();
                    if (mod == "")
                    {
        ...
        public static IEnumerator BuildResAsync(IList<string> assets, IEditorWorkProgressShower winprog, IList<IResBuilderEx> runOnceExBuilder)
        {
            ...
            try
            {
                ...
                // Generate Build Work
                Dictionary<string, ICapsResBuildWork> works = new Dictionary<string, ICapsResBuildWork>();
                ...
                ICapsResBuildWork maniBuildWork = BuilderImp.CreateWork();
