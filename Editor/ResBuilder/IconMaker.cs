...
namespace Capstones.UnityEditorEx
{
    public static class IconMaker
    {
        public static bool WriteTextToImage(string text, string imagepath)
        {
            try
            {
                var thisfile = CapsEditorUtils.__ASSET__;
        ...
        public static bool ChangeImageToIco(string srcpath, string icopath)
        {
            ...
            try
            {
                ...
                si.FileName = System.IO.Path.GetFullPath(CapsModEditor.GetPackageOrModRoot(CapsEditorUtils.__MOD__)) + "/~Tools~/BitmapToIcon.exe";
                si.Arguments = "\"" + srcpath + "\"";
                if (CapsEditorUtils.ExecuteProcess(si))
                {
